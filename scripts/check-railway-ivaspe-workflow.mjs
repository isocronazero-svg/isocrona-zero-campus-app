...
assertNotMatches(/RAILWAY_API_TOKEN/, "Workflow must use only RAILWAY_TOKEN for Railway auth");
assertNotMatches(/exactly one of RAILWAY_API_TOKEN or RAILWAY_TOKEN/);
assertIncludes("Missing required GitHub secret RAILWAY_TOKEN.");
assertIncludes('if [ -z "${RAILWAY_TOKEN:-}" ]; then');
...
for (const line of lines) {
  ...
  assert.ok(
    !/echo .*(\$RAILWAY_TOKEN|\$\{RAILWAY_TOKEN|\$RAILWAY_SSH_PRIVATE_KEY|\$\{RAILWAY_SSH_PRIVATE_KEY)/.test(line),
