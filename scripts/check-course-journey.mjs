...
async function main() {
  ...
  try {
    ...
    const importPath = "/api/test-zone/import";
    assert.equal((await guest.request("GET", importPath, undefined, true)).status, 401);
    assert.equal((await member.request("POST", importPath + "/apply", { part: "IVASPE", bundled: true }, true)).status, 403);
    const beforeImport = (await admin.request("GET", "/api/state")).body;
    const previewImport = (await admin.request("POST", importPath + "/preview", { part: "IVASPE", bundled: true })).body;
    assert.equal(previewImport.ready, 175);
    assert.equal(previewImport.errors.length, 0);
    assert.equal((await admin.request("GET", "/api/state")).body.testZoneQuestions.length, beforeImport.testZoneQuestions.length, "Revisar no escribe");
    const imported = (await admin.request("POST", importPath + "/apply", { part: "IVASPE", bundled: true })).body;
    assert.equal(imported.ready, 175);
    const repeatImport = (await admin.request("POST", importPath + "/apply", { part: "IVASPE", bundled: true })).body;
    assert.equal(repeatImport.ready, 0); assert.equal(repeatImport.duplicates, 175);
    const bankInfo = (await admin.request("GET", importPath)).body;
    assert.ok(bankInfo.blocks.some(b => b.part === "TEMARIO COMÚN"));
    assert.ok(bankInfo.blocks.some(b => b.part === "GUADALAJARA"));
    assert.equal(bankInfo.blocks.find(b => b.part === "IVASPE").topics.length, 6);
    const goodCsv = bankInfo.template.replace("Pregunta de ejemplo", "Pregunta temporal nueva");
    const badBatch = await admin.request("POST", importPath + "/apply", { part: "GUADALAJARA", files: [{ name: "correcto.csv", csv: goodCsv }, { name: "incorrecto.csv", csv: "wrong header\nwrong row" }] }, true);
    assert.equal(badBatch.status, 400);
    assert.equal((await admin.request("GET", importPath)).body.blocks.find(b => b.part === "GUADALAJARA").count, 0, "Un error impide toda la carga");
    await admin.request("POST", importPath + "/apply", { part: "TEMARIO COMÚN", files: [{ name: "comun.csv", csv: goodCsv }] });
    assert.equal((await admin.request("GET", importPath)).body.blocks.find(b => b.part === "TEMARIO COMÚN").count, 1);
    const afterImport = (await admin.request("GET", "/api/state")).body;
    assert.deepEqual(afterImport.associates, beforeImport.associates);
    const importedQuestions = (await member.request("GET", "/api/test-zone/questions")).body.questions;
    const forestQuestion = importedQuestions.find(q => q.part === "IVASPE" && q.category === "Incendios forestales");
    assert.ok(forestQuestion); assert.equal(forestQuestion.correctIndex, undefined);
    const correct = afterImport.testZoneQuestions.find(q => q.id === forestQuestion.id).correctIndex;
    const practice = await member.request("POST", "/api/test-zone/results", { questionIds: [forestQuestion.id], answers: [correct], filters: { part: "IVASPE", category: "Incendios forestales" } });
    assert.equal(practice.body.result.percentage, 100, "Las preguntas importadas se pueden contestar y corregir");
