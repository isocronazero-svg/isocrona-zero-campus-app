...
export function evaluateTest(testRun, answers = [], role = "member") {
  ...
  return {
    ...
    expectedAccountId: testRun?.accountId,
    penaltyDivisor: testRun?.penaltyDivisor || 0,
    timeLimitSeconds: testRun?.timeLimitSeconds || 0,
...
export async function saveTestResult(result) {
  const payload = await fetchJson("/api/test-zone/results", {
    ...
    body: JSON.stringify({
      ...
      expectedAccountId: result?.expectedAccountId,
      penaltyDivisor: result?.penaltyDivisor || 0,
      timeLimitSeconds: result?.timeLimitSeconds || 0,
